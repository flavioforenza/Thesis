...
dir_path = [path for path in os.walk("networks/")]
        ...
        if accepted in current_path[0]:
            ...
            for file in current_path[2]:
                if '.onnx' in file and '.engine' not in file:
                    lst_networks.append(current_path[0].replace('networks/',""))
...
for i in range (0, len(lst_networks)):
    ...
    #save results
    dataframe_display.to_csv("semantic_seg_bench_cv2/network: " +  lst_networks[i] + " display")
